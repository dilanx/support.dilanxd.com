...
export const errorCode = new URLSearchParams(window.location.search).get('e');
...
If neither of those work, submit a **[support request]{`(https://forms.dilanxd.com/paper-support?t=Technical+Issue+or+Bug${errorCode ? `&e=${errorCode}` : ''})`}**.
